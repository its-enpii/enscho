...
export async function getSchoolConfig() {
  try {
    ...
    if (socialMedia.length === 0) {
       if (config.facebook) socialMedia.push({ platform: "Facebook", url: config.facebook });
       if (config.instagram) socialMedia.push({ platform: "Instagram", url: config.instagram });
       if (config.youtube) socialMedia.push({ platform: "YouTube", url: config.youtube });
       if (config.tiktok) socialMedia.push({ platform: "TikTok", url: config.tiktok });
    }
    ...
    return {
       ...
       socials: { 
          ...
          facebook: socialMedia.find(s => s.platform.toLowerCase() === 'facebook')?.url || "",
          instagram: socialMedia.find(s => s.platform.toLowerCase() === 'instagram')?.url || "",
          youtube: socialMedia.find(s => s.platform.toLowerCase() === 'youtube')?.url || "",
          tiktok: socialMedia.find(s => s.platform.toLowerCase() === 'tiktok')?.url || "",
       }
    };
  } catch (error) {
