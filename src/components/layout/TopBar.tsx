...
import { Facebook, Instagram, Youtube, Twitter, Linkedin, Github, Globe, Phone, User, LogIn } from "lucide-react";
...
const SocialIcon = ({ platform }: { platform: string }) => {
  ...
  if (p.includes("tiktok")) return <span className="font-bold text-[10px]">TT</span>; // Lucide might not have TikTok
...
export function TopBar({ schoolInfo }: TopBarProps) {
  ...
  const socials = schoolInfo.socialMedia && schoolInfo.socialMedia.length > 0 
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3 border-l border-slate-700 pl-4">
             {socials.map((social, idx) => (
                <Link key={idx} href={social.url} target="_blank" className="hover:text-blue-500 transition-colors" title={social.platform}>
        ...
        <div className="flex items-center gap-4">
          <div className="group relative">
            ...
            <div className="absolute right-0 mt-2 w-48 bg-white text-slate-800 rounded-md shadow-lg py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
              <Link href="/login/siswa" className="block px-4 py-2 hover:bg-slate-100 flex items-center gap-2">
                <LogIn size={14} /> E-Learning Siswa
              </Link>
              <Link href="/login/guru" className="block px-4 py-2 hover:bg-slate-100 flex items-center gap-2">
                <User size={14} /> Portal Guru
              </Link>
              <Link href="/login/alumni" className="block px-4 py-2 hover:bg-slate-100 flex items-center gap-2">
                <User size={14} /> Akses Alumni
              </Link>
            </div>
          </div>
          <div className="border-l border-slate-700 pl-4">
             <span className="text-xs font-semibold bg-slate-800 px-2 py-1 rounded">ID</span>
          </div>
        </div>
      </div>
    </div>
