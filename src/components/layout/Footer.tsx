...
export function Footer({ schoolInfo, majors }: FooterProps) {
  ...
  const getSocialLink = (platform: string) => {
     if (schoolInfo.socialMedia && schoolInfo.socialMedia.length > 0) {
        const found = schoolInfo.socialMedia.find((s: any) => s.platform.toLowerCase() === platform.toLowerCase());
        return found ? found.url : "";
     }
     // Fallback to legacy
     return schoolInfo.socials?.[platform as keyof typeof schoolInfo.socials] || "";
  };
    ...
    <footer className="bg-slate-900 text-slate-300 pt-16 pb-8 text-sm">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          {/* Column 1: About */}
          <div>
            <h3 className="text-white text-lg font-bold mb-4">{schoolInfo.name}</h3>
            <p className="mb-6 leading-relaxed text-slate-400">
              {schoolInfo.footerDescription || "Mewujudkan generasi unggul yang siap kerja, cerdas, dan berkarakter melalui pendidikan vokasi berkualitas."}
            </p>
            ...
            <div className="flex gap-4">
               {facebookUrl && (
                  <a href={facebookUrl} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-colors">
                     <Facebook size={16} />
                  </a>
               )}
               {instagramUrl && (
                  <a href={instagramUrl} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center hover:bg-pink-600 hover:text-white transition-colors">
                     <Instagram size={16} />
                  </a>
               )}
               {youtubeUrl && (
                  <a href={youtubeUrl} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center hover:bg-red-600 hover:text-white transition-colors">
                     <Youtube size={16} />
                  </a>
               )}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[
                ...
                <li key={item.label}>
                   <Link href={item.href} className="hover:text-blue-400 transition-colors block py-1 flex items-center gap-2">
          ...
          <div>
            <h3 className="text-white text-lg font-bold mb-4">Kompetensi Keahlian</h3>
            <ul className="space-y-2">
              {majors.length > 0 ? majors.map((major) => (
                  <li key={major.slug}>
                     <Link href={`/jurusan/${major.slug}`} className="hover:text-blue-400 transition-colors block py-1 flex items-center gap-2">
                       <span className="w-1 h-1 rounded-full bg-blue-500"></span>
                       {major.name}
                     </Link>
                  </li>
              )) : (
                 <li className="text-slate-500 italic">Belum ada data jurusan.</li>
              )}
            </ul>
          </div>
                 ...
                 <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-blue-500 mt-1 shrink-0">
                  ...
                  <div>
                    ...
                    {schoolInfo.openingHours ? (
                       ...
                       <>
                         <p className="text-xs text-slate-400">Senin - Jumat: 07:00 - 16:00</p>
                         <p className="text-xs text-slate-400">Sabtu: 07:00 - 12:00</p>
                       </>
                    )}
                  </div>
               </div>
