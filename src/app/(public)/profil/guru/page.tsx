    ...
    <div className="bg-slate-50 min-h-screen">
       ...
       <div className="bg-slate-900 text-white py-20 relative overflow-hidden">
         ...
         <div className="container mx-auto px-4 relative z-10 text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Dewan Guru & Staf</h1>
            <p className="text-slate-300 max-w-2xl mx-auto text-lg">
              Mengenal lebih dekat para pendidik dan tenaga kependidikan SMK Enscho yang berdedikasi.
            </p>
         </div>
       </div>
       ...
       <div className="container mx-auto px-4 py-16 space-y-20">
          ...
          {kepsek.length > 0 && (
            <section className="text-center">
               ...
               <div className="flex justify-center">
                  {kepsek.map(p => (
                     <div key={p.id} className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 max-w-sm w-full hover:-translate-y-2 transition-transform duration-300">
                        ...
                        <p className="text-blue-600 font-medium">{p.position || "Kepala Sekolah"}</p>
                     </div>
                  ))}
               </div>
            </section>
          ...
          {teachers.length > 0 && (
            <section>
               <h2 className="text-3xl font-bold text-slate-800 mb-10 text-center">Guru & Pengajar</h2>
          ...
          {staff.length > 0 && (
            <section>
               <h2 className="text-3xl font-bold text-slate-800 mb-10 text-center">Tata Usaha & Staf</h2>
               <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
                  {staff.map(s => (
           ...
           {employees.length === 0 && (
             <div className="text-center py-20 text-slate-500">
                ...
                <p className="text-lg">Belum ada data guru dan staf yang ditampilkan.</p>
             </div>
           )}
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow group">
       ...
       <div className="p-5 text-center">
          <h3 className="font-bold text-slate-900 mb-1 line-clamp-1" title={employee.name}>{employee.name}</h3>
          <p className="text-sm text-slate-500 line-clamp-1">{employee.position || "-"}</p>
       </div>
    </div>
  )
}
